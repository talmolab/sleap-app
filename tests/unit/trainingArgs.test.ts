...
describe("hydraQuote", () => {
  ...
  it("preserves an embedded '=' (the run-name .n= case) inside the quotes", () => {
...
describe("buildTrainingArgs — Hydra override safety", () => {
  it("REGRESSION: default run name's `.n=<count>` suffix is quoted, not bare", () => {
  ...
  it("labels path is a quoted Hydra list element", () => {
    expect(buildTrainingArgs(base)).toContain(
      "data_config.train_labels_path=['/Users/a/flies.slp']",
    );
  });

  it("a project path containing '=' is quoted so the path overrides survive", () => {
  ...
  it("no override value contains a bare (unquoted) second '='", () => {
