...
| `/docs/dev/` | tracks `main` at the nightly's cadence | with the nightly dev build (same run that deploys app `/dev/`) |
| `/docs/main/` | tracks `main` per commit | every push to `main` (same run that deploys app `/main/`) |
...
- **In-app Help links follow the channel the app is on**, resolved at click time by `getDocsUrl()` / `useDocsUrl()` in `src/lib/docsUrl.ts` (pure mappings in `src/lib/version.ts`). Three call sites: `MenuBar.tsx` (Help → Documentation), `MenuSearchDialog.tsx` (command palette), `LabelingTipsDialog.tsx`.
  - **Web** reads the channel path it is served under (Vite's `BASE_URL`, set per target as `VITE_BASE_PATH`): `/latest/` → `/docs/latest/`, `/dev/` → `/docs/dev/`, `/main/` → `/docs/main/`, root → `/docs/stable/` — each written by the same deploy run that built that app path, so no channel's docs lag its app. `/main/` and `/dev/` are separate folders precisely because they move on different cadences (per commit vs. nightly); one shared folder would have to lag one of them. Only one path doesn't map to a like-named folder: a permanent *pre-release* path like `/v0.1.2-2/` → `/docs/latest/` (pre-releases get no pinned folder). A permanent *stable* path keeps its own pinned docs: `/v0.1.1/` → `/docs/v0.1.1/`.
  ...
  - **Running from source** (`bun run dev`, `bun run tauri:dev`) always resolves to `/docs/main/`: you are working off `main`, and the committed `package.json` version drifts since nothing on `main` bumps it. `/docs/main/` is rebuilt on every push, so it is the closest published docs to your tree. The `import.meta.env.DEV` check is `=== true` on purpose — Vite substitutes a real boolean, but `bun test` puts the string `"true"` there, which would otherwise short-circuit every test of the branches above.
...
- **`versions.json` is generated by `deploy.yml`, not by `mike`.** The theme's dropdown only needs that file in mike's format; `extra.version.provider: mike` in `mkdocs.yml` names the *format*, not the tool. Avoiding mike keeps a second writer off the `gh-pages` branch, which `deploy.yml` serializes on `concurrency: deploy-pages`. Every landable path needs an entry, the unversioned pointers (`stable`, `latest`, `dev`, `main`) included — an unlisted path silently mislabels itself as `extra.version.default`.
