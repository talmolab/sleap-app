...
- **In-app Help links are derived per build**, not hardcoded: `docsUrlForVersion()` in `src/lib/version.ts` maps the stamped `__APP_VERSION__` to a docs path, so a stable build pins to its own `/docs/v<tag>/` and never drifts onto docs for features it lacks. Three call sites: `MenuBar.tsx` (Help → Documentation), `MenuSearchDialog.tsx` (command palette), `LabelingTipsDialog.tsx`.
